...
function useInitialArch(level, group, setGroup, ungroup, setUngroup,checkNum, setCheckNum) {
...
//          console.log(jsonData);
        ...
        // ------ Custom Group ------\\
         ...
         // ------ level 2 or level3 누른 후 level1 돌아올 때------\\
        else if(level ===1 && checkFirst === 2){

        }
        // ------ Auto group level 2 or level3 (처음 누르는 경우) ------\\
        else if (level !== 1){
            console.log("// ------ Auto group level 2 or level3 ------\\");
            if(checkFirst === 2){
                console.log("level2, 3 두번 이상 눌렀을 경우");

            }

            else{
                console.log("level 2, 3 처음 눌렀을 경우");
            }
              //그룹 정보 삭제하기
            for (var j=0; j<20; j++){
              axios.delete('/api/sort_group/'.concat(j).concat('/'))
    ...
    init();
  }, [level, group, setGroup, ungroup, setUngroup, checkNum, setCheckNum]);
